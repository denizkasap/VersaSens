...
int FDC2214_configure(FDC_2214 *dev)
{
    ...
    if (dev->channel_mask & 0x01) {
        data2write = 0x012C;
        status |= FDC2214_write_16bit(REG_FDC_SETTLECOUNT_CH0, data2write, dev->sensor_address);
        data2write = dev->sampling_rate;
        status |= FDC2214_write_16bit(REG_FDC_RCOUNT_CH0, data2write, dev->sensor_address);
        data2write = 0x0000;
        status |= FDC2214_write_16bit(REG_FDC_OFFSET_CH0, data2write, dev->sensor_address);
        data2write = 0x2001;
        status |= FDC2214_write_16bit(REG_FDC_CLOCK_DIVIDERS_CH0, data2write, dev->sensor_address);
        data2write = 0xF800;
    ...
    if (dev->channel_mask & 0x02) {
        data2write = 0x012C;
        status |= FDC2214_write_16bit(REG_FDC_SETTLECOUNT_CH1, data2write, dev->sensor_address);
        data2write = dev->sampling_rate;
        status |= FDC2214_write_16bit(REG_FDC_RCOUNT_CH1, data2write, dev->sensor_address);
        data2write = 0x0000;
        status |= FDC2214_write_16bit(REG_FDC_OFFSET_CH1, data2write, dev->sensor_address);
        data2write = 0x2001;
        status |= FDC2214_write_16bit(REG_FDC_CLOCK_DIVIDERS_CH1, data2write, dev->sensor_address);
        data2write = 0xF800;
    ...
    if (dev->channel_mask & 0x04) {
        data2write = 0x012C;
        status |= FDC2214_write_16bit(REG_FDC_SETTLECOUNT_CH2, data2write, dev->sensor_address);
        data2write = dev->sampling_rate;
        status |= FDC2214_write_16bit(REG_FDC_RCOUNT_CH2, data2write, dev->sensor_address);
        data2write = 0x0000;
        status |= FDC2214_write_16bit(REG_FDC_OFFSET_CH2, data2write, dev->sensor_address);
        data2write = 0x2001;
        status |= FDC2214_write_16bit(REG_FDC_CLOCK_DIVIDERS_CH2, data2write, dev->sensor_address);
        data2write = 0xF800;
    ...
    if (dev->channel_mask & 0x08) {
        data2write = 0x012C;
        status |= FDC2214_write_16bit(REG_FDC_SETTLECOUNT_CH3, data2write, dev->sensor_address);
        data2write = dev->sampling_rate;
        status |= FDC2214_write_16bit(REG_FDC_RCOUNT_CH3, data2write, dev->sensor_address);
        data2write = 0x0000;
        status |= FDC2214_write_16bit(REG_FDC_OFFSET_CH3, data2write, dev->sensor_address);
        data2write = 0x2001;
        status |= FDC2214_write_16bit(REG_FDC_CLOCK_DIVIDERS_CH3, data2write, dev->sensor_address);
        data2write = 0xF800;
    ...
    status |= FDC2214_write_16bit(REG_FDC_MUX_CONFIG, mux, dev->sensor_address);
    if (status != 0){
    ...
    data2write = 0x1E81;
    status |= FDC2214_write_16bit(REG_FDC_CONFIG, data2write, dev->sensor_address); 
    if (status != 0){
...
uint32_t FDC2214_get_values(FDC_2214 *dev, uint8_t channel_id){
    ...
    */
    ...
    int res = FDC2214_read_16bit(REG_FDC_STATUS, &conv_status, dev->sensor_address);
    ...
    while (!(conv_status & unread_conv)) {
        ...
        FDC2214_read_16bit(REG_FDC_STATUS, &conv_status, dev->sensor_address);
    }

    uint16_t msb_value = 0;
    uint16_t lsb_value = 0;
    res = 0;
    res |= FDC2214_read_16bit(add_MSB, &msb_value, dev->sensor_address);
    cap_value = ((uint32_t)(msb_value & 0x0FFF) << 16);
    res |= FDC2214_read_16bit(add_LSB, &lsb_value, dev->sensor_address);
    cap_value |= lsb_value;
